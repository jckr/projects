var h=250,w=380,bw=75,ch=200;
var svg=d3.select("#chart").append("svg").attr("height",h).attr("width",w);
...
var cats=["Total", "OECD", "Nordic countries","Latin America", "Males", "Females", "15-24 years old", "65+ years old"];
var cat=0;


var y=d3.scale.linear().domain([.99,1.01]).range([ch,0]);
var x=d3.scale.ordinal().domain(d3.range(indicators.length)).rangeRoundBands([0,w],.1);
var data;
...
d3.csv("simple.csv",function(csv){
	d3.select("#countries")
		.select("ul")
		.selectAll("li")
		.data(cats)
		.enter()
		.append("li")
			.classed("active",function(d,i) {return i==cat;})
			.append("a")
				.attr("href","#")
				.html(String)
				.on("click",function(d,i) {cat=i;d3.select("#countries").selectAll("li").classed("active",function(d,i) {return i==cat;});init();})
	
	indicators.forEach(function(d,i) {
		d.values=csv.map(function(c) {return +c["n"+(i+1)];});
	})

	var axis=svg.append("line").attr("x1",0).attr("x2",w).attr("y1",y(1)).attr("y2",y(1)).style("stroke","grey").style("opacity",0);
	var circles=svg.selectAll(".indicatorsg").data(indicators).enter()
		.append("circle")
		.attr("cx",function(d,i) {return x(i)+10;})
		.attr("r",0)
		.attr("cy",400).style("fill",function(d,i) {return d.color;});
	var lines=svg.selectAll(".indictorsg").data(indicators).enter()
		.append("line")
		.attr("x1",function(d,i) {return x(i)+10;})
		.attr("x2",function(d,i) {return x(i)+10;})
		.attr("y1",y(1))
		.attr("y2",y(1))
		.style("stroke",function(d,i) {return d.color;})
		.style("stroke-width",3)
		.style("opacity","0");
	var texts=svg.selectAll("text").data(indicators).enter()
		.append("text")
		
		.attr("transform",function(d,i) {return "rotate(-90) translate(-240,"+(x(i)+14)+")";})
	...
	{
		circles.transition()
			.attr("cy",function(d) {return y(d.values[cat]);})
			.attr("r",5);
		lines.transition()
			.attr("y2",function(d) {return y(d.values[cat]);})
			.style("opacity",.5);
		axis.transition()
			.style("opacity",function() {return cat?.5:0;})
	}
