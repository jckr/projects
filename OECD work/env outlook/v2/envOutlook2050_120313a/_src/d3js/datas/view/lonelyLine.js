...
var mycolor = d3.scale.linear().domain(scales[0].domain).range(colourRange);
...
function drawStart() {
	defCircle(circles.append("svg:circle"), 0, currentColor);
...
function drawSegment(mySeries,s) {
	if (s>=mySeries.length-1) {
		...
		lines.append("svg:line")
			...
			.each("end",function() {
				// draw me a circle
				defCircle(circles.append("svg:circle"), s+1, currentColor);
...
function moveTo(view,scenario) {
		...
		mycolor.domain(scales[view].domain);
...
function defCircle(circle, index, color) {
	
	// first unifying the first circle & the pack
	var deltaI = (index == 0) ? index : index-1;
	
	circle.attr("cx",x(mySeries[index].year)) // probably on the y axis, but hey
		.attr("cy",y(mySeries[index].value))
		.style("stroke","white").style("stroke-width",sB).style("fill",color)
		.attr("r",0)
		.attr("class","circle c"+deltaI)
	...
	circle.append("svg:title").text(function(d) {return mySeries[index].year+": "+mySeries[index].value.toFixed(1);})
...
function updateCircle (parent, mySerie) {
	...
	parent.selectAll("title").remove();circles.selectAll("original-title").remove();
