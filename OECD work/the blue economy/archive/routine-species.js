...
d3.select("#containers").on("click",function() {
...
function move() {
  ...
  nodes.forEach(function(node) {
  	...
  	node.x=xy[0];
  	node.y=xy[1];
  	node.r1=Math.sqrt(node.ton/(1500000/projection.scale()));
	node.r2=Math.sqrt(node.teu/(50*projection.scale()));
  })
  
  circles
  	.attr("cx",function(d){return d.x;})
  	.attr("cy",function(d){return d.y;})
  	.attr("r",function(d){return mode?d.r2:d.r1;})
