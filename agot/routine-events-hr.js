...
var bSize=140*2,intL=(920*2)-5*(bSize);
var thVis=480*2,tvVis=340*2;
var wIC=[
...
defs.append("path").attr({id:"gUp",d:"M-650,0 A650,650 0 0,1 650,0"});	// to write on circles.
defs.append("path").attr({id:"gDown",d:"M-670,0 A670,670 0 0,0 670,0"}); 
		...
		d3.csv("characters.csv",function(characters) {
			...
			d3.csv("events.csv",function(events) {
				...
				function init() {
					...
					vis.append("g").attr("id","legend").attr("transform","translate("+(2*755-thVis)+","+(2*550-tvVis)+")");
					calculate();
				...
				function interface() {
					var myInt=svg.append("g").attr({id:"interface",transform:"translate(0,1330)"});
					myInt
					.append("text")
					.attr({x:20,y:360})
					...
					var bookButtons=myInt.selectAll(".bookButton").data(books).enter().append("g").classed("bookButton",1).attr("transform",function(d,i) {return "translate("+(intL+bSize*i)+",326)";})
					...
					.attr("d",function(d,i) {
						if(i==0){
							return "M0,10 Q0,0 10,0 h "+(bSize-10)+"l20,20l-20,20h-"+(bSize-10)+"Q0,40 0,30  Z";
						}
						if(i<4){
							return "M0,0h"+bSize+"l20,20l-20,20h-"+bSize+"l20,-20Z";
						}
						return "M0,0h"+(bSize-10)+"Q"+bSize+",0 "+bSize+",10 v20 Q"+bSize+",40 "+(bSize-10)+",40h-"+(bSize-10)+"l20,-20Z";
					})
					...
					var timelineBooks=myInt.selectAll(".timelineBooks").data(books).enter().append("rect").classed("timelineBooks",1)
						.attr({
							x:function(d,i) {return (i>bookSelected)?(922*2):(intL+(d.first/maxStep)*(5*bSize)+2)},
					...
					bookButtons.append("text").text(function(d) {return d.title;}).attr("y",28).attr("x",30).style("font-size","20px");
					...
					var charBlock=myInt.append("g").attr({"id":"charBlock"}).attr("transform","translate(0,100)");
					charBlock.append("rect").attr({id:"emptyCharInt",width:5*bSize,height:150,x:intL,y:-30,rx:10,ry:10}).style({fill:"#eee",stroke:"#ddd"});
					charBlock.append("text").attr({id:"clickme",x:1134,y:20,"text-anchor":"middle"}).text("Click a circle above for information about a character").style({"font-style":"italic","font-family":"Open Sans"})
					charInt=charBlock.append("g").attr("id","charInt");
					var charFullName=charInt.append("text").attr({x:20,y:10});
					charFullName.append("tspan").attr("id","charPrefix").style({"font-style":"italic","font-size":"24px"})
					charFullName.append("tspan").attr("id","charName").style({"font-weight":"normal","font-size":"24px"})
					var charFullTeam=charInt.append("text").attr({x:20,y:-30}).style({"font-style":"normal","font-size":"18px"})
					...
					charInt.append("text").attr({id:"charTitle",x:20,y:40}).style({"font-weight":"normal","font-size":"24px"})
					charInt.append("text").attr({id:"charStatus",x:20,y:70}).style({"font-family":"Open Sans","font-size":"24px"})
					
					charInt.append("g").attr({id:"charTimeLine",transform:"translate("+intL+",40)"}).append("path").attr("d","M5,3v-3h"+(5*bSize-10)+"v3").style({"stroke":"#ccc","fill":"none"});

					myInt.append("text").text("Character details:").style({"font-family":"Open Sans","font-weight":"bold"}).attr({x:20,y:12,id:"cd"});
					myInt.append("text").text("Time control:").style({"font-family":"Open Sans","font-weight":"bold"}).attr("id","cd").attr({id:"tc",x:20,y:220});
...
function halfchord(d) {
	return d3.min([200,d3.max([0.1,r*Math.sin(d.angle/2)])]);
}
...
/*console.log("---")
	...
	if(charHighlighted>-1) {
		var cTimeScale=d3.scale.linear().domain([0,maxStep]).range([10,5*bSize-10]);
		...
		markers.exit().remove();
		markers.enter().append("g").classed("markers",1).append("circle").attr({"cy":0,"r":5}).style({"fill":"#222","opacity":.5});
		...
		myEvents.enter().append("circle").classed("markEvent",1)
			.attr({cy:function(d,i) {return -(i+1)*20;},r:5}).style("fill",function(d) {return colors[d.type];})
		...
		.attr("d",function() {return "M"+(intL+cTimeScale(i))+",-30v150";});
	} else {
		charInt=d3.select("#interface").select("#charInt");
...
function drawModal(d,j) {
	...
	.transition()
	.style({left:(intL+(d.time/maxStep)*5*bSize+5)+"px",bottom:350+j*10+"px"})
	}
...
function drawMentionedModal(d) {
	...
	transition().style({left:x+"px",bottom:"350px"})
}
function drawGroupModal(d,j) {
	var x=340*2*Math.cos(d.pos)+thVis+5;
	var y=340*2*Math.sin(d.pos)+tvVis-20+50;
