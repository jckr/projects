					...
					eve.forEach(function(d,i) {		// we will loop one per chapter
						...
						//console.log(e);
							...
							if(e.event==="killed") {
								...
								if(e.withID!=="") {
									myTime.links.push({killer:e.withID,kindex:myTime.charHash[e.withID],victim:id,vindex:myTime.charHash[id],chapter:c});
									...
									charTimes[myTime.charHash[e.withID]].kills.push({index:myTime.charHash[e.id],time:c,bodycount:charTimes[myTime.charHash[e.withID]].bodycount})
						...
						myTime.links.forEach(function(d) {pusher.links.push({killer:d.killer,kindex:d.kindex,victim:d.victim,vindex:d.vindex,chapter:d.chapter});})
						myTime.events.forEach(function(d) {pusher.events.push({text:d.text,type:d.type});})
						
						pusher.groupKills=d3.range(5).map(function() {return d3.range(5).map(function() {return 0;})});
						myTime.links.forEach(function(link) {//console.log(link);
							killerGroup=groupHash[myTime.characters[link.kindex].team];	
							victimGroup=groupHash[myTime.characters[link.vindex].team];
							pusher.groupKills[killerGroup][victimGroup]=pusher.groupKills[killerGroup][victimGroup]+1;
})
						...
						pusher.links.forEach(function(d) {
							...
							var killerTeam=killer.team,killerGroup=groupHash[killerTeam];
							var victimTeam=victim.team,victimGroup=groupHash[victimTeam];
							...
							if(victimGroup==killerGroup){
								d.attack=d.path;
								d.victim="M"+d.x2+","+d.y2;
							} else {
								gAPx=Math.cos(pusher.groups[killerGroup].pos+.1)*100; // group attack point
								gAPy=Math.sin(pusher.groups[killerGroup].pos+.1)*100;
								gVPx=Math.cos(pusher.groups[victimGroup].pos-.1)*100; // group victim point
								gVPy=Math.sin(pusher.groups[victimGroup].pos-.1)*100;

								d.attack="M"+d.x1+","+d.y1+" Q "+((d.x1+gAPx)/2)+","+((d.y1+gAPy)/2)+" "+gAPx+","+gAPy+" L"+gAPx+","+gAPy;
								d.victim="M"+gVPx+","+gVPy+" Q "+((d.x2+gVPx)/2)+","+((d.y2+gVPy)/2)+" "+d.x2+","+d.y2+" L"+d.x2+","+d.y2;							}
...
/*console.log("---")
	...
	linkVectors=[];
	times[i].groupKills.forEach(function(d,j) {d.forEach(function(kill,k) {linkVectors.push({killer:j,victim:k,kills:kill});})})

	newlinksA=vis.selectAll(".attack").data(times[i].links).enter().append("path").classed("link attack",1).attr("d",function(d) {return d.attack;}).style("stroke","red")
	newlinksV=vis.selectAll(".victim").data(times[i].links).enter().append("path").classed("link victim",1).style("stroke","green")
	vis.selectAll(".kill").data(linkVectors)
	.enter().append("path").classed("link kill",1).attr("d",function(d) {
		console.log(d);
		var gAPx=Math.cos(times[i].groups[d.killer].pos+.1)*100; // group attack point
		gAPy=Math.sin(times[i].groups[d.killer].pos+.1)*100;
		gVPx=Math.cos(times[i].groups[d.victim].pos-.1)*100; // group victim point
		gVPy=Math.sin(times[i].groups[d.victim].pos-.1)*100;
		return"M"+gAPx+","+gAPy+" Q "+((gAPx+gVPx)/4)+","+((gAPy+gVPy)/4)+" "+gVPy+","+gVPy+" L"+gVPx+","+gVPy;

	}).style("stroke","blue");
	
	// links transition
	d3.selectAll(".kill").transition().attr("d",function(d) {
		var gAPx=Math.cos(times[i].groups[d.killer].pos+.1)*100; // group attack point
		gAPy=Math.sin(times[i].groups[d.killer].pos+.1)*100;
		gVPx=Math.cos(times[i].groups[d.victim].pos-.1)*100; // group victim point
		gVPy=Math.sin(times[i].groups[d.victim].pos-.1)*100;
	...
	.style("visibility",function(d) {return (d.kills&&(d.killer!=d.victim))?"visible":"hidden";})
	.style("stroke-width",function(d) {return d.kills;})
	d3.selectAll(".attack").attr("d",function(d) {return d.attack;})
	d3.selectAll(".victim").attr("d",function(d) {return d.victim;})
	d3.selectAll(".victim").exit().remove();

	d3.selectAll(".attack").exit().remove();
