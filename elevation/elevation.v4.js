(function() {
	...
	vis.drawCoast = function() {
		d3.select("#landmass").selectAll("path").data(vis.coastPath).enter().append("path")
			.attr("d", function(d) {
				myPath = ""
				d.values.forEach(function(v, i) {
	...
	vis.draw = function() {
		svg.selectAll("rect.back").data([{}]).enter().append("rect")
		.classed("back", 1)
		.attr({width: 2 * width, height: 2 * height}).style("fill", "pink")
		//.style("clip-path", "url(#landmass)")
		...
		*/
		svg.selectAll("g.zones").data([{}]).enter().append("g").classed("zones", 1)
		.style("clip-path", "url(#landmass)");
		svg.selectAll("g.streets").data([{}]).enter().append("g").classed("streets", 1);
		svg.selectAll("g.path").data([{}]).enter().append("g").classed("path", 1);
		svg.selectAll("g.click").data([{}]).enter().append("g").classed("click", 1);


		svg.selectAll("g.streets *, g.zones *, g.path *").remove();
		

		svg.select("g.streets").selectAll("line.street").data(vis.edgesArray).enter()
			.append("line").classed("street", 1)
				...
				.style("stroke-width", 3) //function(d) {return d3.min([8, 3 * d.lanes]) || 6;})
		...
		svg.select("g.streets").selectAll("line.sp").data(vis.shortestPath).exit().remove();
		svg.select("g.streets").selectAll("line.sp").data(vis.shortestPath).enter()
			.append("line").classed("sp", 1)
					.attr({
						x1: function(d) {return vis.points[d.start].x},
						x2: function(d) {return vis.points[d.end].x},
						y1: function(d) {return vis.points[d.start].y},
					...
					.style("stroke-width", 1) //function(d) {return d3.min([8, 3 * d.lanes]) || 6;})
					.style("stroke", "#222")
					.style("stroke-opacity", 1)
					.style("stroke-linecap", "round")

	/*	svg.select("g.path").selectAll("circle").data(vis.points.filter(function(d) {return d.dist && d.dist < Infinity;})).exit().remove();
		svg.select("g.path").selectAll("circle").data(vis.points.filter(function(d) {return d.dist && d.dist < Infinity;}))
			...
			.style({
				...
				stroke: "none",
				fill: function(d) {
					var lvl = 64 * (1 + d.dist % 4) - 1;
					return d3.rgb(lvl, lvl, lvl);
				}
			})
			.on("click", function(d) {console.log(d)})*/

		svg.select("g.zones").selectAll("path").remove();
		svg.select("g.zones").selectAll("path").data(vis.rings.slice(0,130).reverse()).enter()
			.append("path")
			.attr("d", function(d) {
				var path = ""
				d.values.forEach(function(v, i) {
					path = path + (i ? "L" : "M") + vis.points[v.id].x + "," + vis.points[v.id].y;
				})
				return path + "Z";
			})
			.style("fill", function(d, i) {
				return ["#777", "#999", "#aaa", "#ccc", "#eee"][(i + 4) % 5];
			})
	...
	vis.recalc = function(source) {
		...
		var x0 = vis.points[source].x, y0 = vis.points[source].y;

		vis.steps = [];
		vis.points.forEach(function(d, i) {
			if (vis.distances[i] < Infinity) {
				var node = i;
				var dist = 0;
				while (node != vis.source) {
					var e = vis.backtrack[node];
				...
				d.angle = ((y1 > y0)? 1 : -1) * Math.acos((x1 - x0) / mapDist);
				d.dist = dist;
				d.idx = i;
				vis.steps.push({dist: dist, id: i});
			}
		})
		vis.rings = d3.nest().key(function(d) {return d.dist;}).entries(vis.steps);
		vis.rings.forEach(function(d) {
			d.values.sort(function(a, b) {
				return vis.points[a.id].angle - vis.points[b.id].angle
			})
		})
