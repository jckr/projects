...
var nbPeople,nbAttractions,minCap,maxCap,step,o,night,timePerVisit,timePerVisitO,tickPerDecision,duration,mode;
...
function readForm() {
	night=+d3.select("#night").property("value");
	d3.select("#night").on("change",function() {night=+d3.select(this).property("value");reset();})

	duration=+d3.select("#duration").property("value");
	d3.select("#duration").on("change",function() {duration=+d3.select(this).property("value");reset();})

	nbPeople=+d3.select("#nbPeople").property("value");
	d3.select("#nbPeople").on("change",function() {nbPeople=+d3.select(this).property("value");reset();})

	step=+d3.select("#stepAvg").property("value");
	d3.select("#stepAvg").on("change",function() {step=+d3.select(this).property("value");reset();})

	stepStd=+d3.select("#stepStd").property("value");
	d3.select("#stepStd").on("change",function() {stepStd=+d3.select(this).property("value");reset();})

	timePerVisit=+d3.select("#timePerVisit").property("value");timePerVisitO=Math.round(timePerVisit/10);
	d3.select("#nbPeople").on("change",function() {timePerVisit=+d3.select(this).property("value");timePerVisitO=Math.round(timePerVisit/10);reset();})

	nbAttractions=+d3.select("#nbAttractions").property("value");cScale.domain([0,10,nbAttractions]);
	d3.select("#nbAttractions").on("change",function() {nbAttractions=+d3.select(this).property("value");cScale.domain([0,10,nbAttractions]);reset();})

	o=+d3.select("#o").property("value");
	d3.select("#o").on("change",function() {o=+d3.select(this).property("value");reset();})

	minCap=+d3.select("#minCap").property("value");
	d3.select("#minCap").on("change",function() {minCap=+d3.select(this).property("value");reset();})

	maxCap=+d3.select("#maxCap").property("value");
	d3.select("#maxCap").on("change",function() {maxCap=+d3.select(this).property("value");reset();})

	mode=d3.select("#mode").property("checked");
	d3.select("#mode").on("change",function() {mode=d3.select(this).property("checked");reset();})
	...
	lyak=d3.select("#lyak").property("checked");
	d3.select("#lyak").on("change",function() {lyak=d3.select("#lyak").property("checked");reset();})

	d3.select("#run").on("click",function() {run(lyak);})
...
function generatePeople() {
	var x=Math.random()*wCity,
		y=Math.random()*hCity,
		start=d3.max([0,(norm()-.5)])*night*.3,
		end=d3.min([night,start+(.2+norm())*night]),
		r=~~(255*norm()),g=~~(255*norm()),b=~~(255*norm()),gr=~~((r+g+b)/3),
		color="#ccc";//rgb(0,0,"+b+")";
		if(!mode){start=0;end=night;}
		...
		return {status:(start>0)?8:0, // if this person's starting point is after the start of the night their initial status is at hom
				...
				stepx:x,
				stepy:y,
				speed:.8*step+.4*step*norm(),
				steps:0,
				tickPerDecision:~~(.8+.4*norm())*tickPerDecision,
				destination:-1,
				nbVisited:0,
				timeInInstall:0,
				color:color,visited:d3.range(nbAttractions).map(function() {return 0;}),
				id:indexPeople
			};
}
...
function generateInstall() {
	var posx=Math.random()*wCity,
		...
		outside=Math.random()<o,
		capacity=minCap+Math.random()*(maxCap-minCap),
		occupancy=0;
		visited=0;
...
function stop() {
	if(!running&&tick){running=1;run(lyak);} // if paused, resume
...
function reset() {
	...
	indexPeople=-1;
	indexInstall=-1;
	people=[]; d3.range(nbPeople).forEach(function() {people.push(generatePeople())});
	installs=[]; d3.range(nbAttractions).forEach(function() {installs.push(generateInstall())});
	...
	total=[0,0,0];
	visitStats=d3.range(nbAttractions).map(function(d) {return 0;})
	visitStats[0]=nbPeople;
...
function run(flag) {
	if(running&&flag!=lyak) {reset();run(flag);}
	lyak=flag;
	if(!running&&tick){running=1;run(lyak);}
...
function update() {
	...
	people.forEach(function(p) {
		if (tick>=p.start) { // it's not time until it's time.
			p.x=p.stepx;p.y=p.stepy; // each step counts. If you moved last tick, it's updated here.
			...
			if(p.status===0) { // seeking installation to visit
				...
				if (lyak) {
					remainingAttr.sort(function (i,j) {return (i.line.length)*timePerVisit/i.capacity+Math.sqrt(dist(p,i))/p.speed-Math.sqrt(dist(p,j))/p.speed-timePerVisit*(j.line.length)/j.capacity;})	
				...
				if (lyak) {
					...
					if ((i.line.length)*timePerVisit/i.capacity+Math.sqrt(dist(p,i))/p.speed+tick>p.end+homeDist(p)/p.speed) {
			...
			if(p.status===1) { // going towards an installation
				p.step+=1;
				/*if(lyak&&!(p.step%p.tickPerDecision)) {
			...
			if(p.status===2) { // trying to enter
				...
				if(i.occupancy<i.capacity || i.outside) { // and we're in! (or out, if an outside installation)
					...
					if(i.line.length*timePerVisit/i.capacity+tick>p.end+homeDist(p)/p.speed) { // line too scary. 
			...
			if(p.status===4) { // enjoying the sight
				...
				if(p.timeInInstall>timePerVisit||i.outside&&p.timeInInstall>timePerVisitO) { // time to go
			...
			if(p.status===5) { // shall we go try another one?
				...
				if (p.nbVisited===nbAttractions) { // home run!
...
function drawGrid() {
	...
	var y=d3.scale.linear().range([0,125]).domain([0,nbPeople]);
	var x=d3.scale.linear().range([0,300]).domain([0,nbAttractions]);
...
;
	...
	var time=svg.append("g").attr("transform","translate(550,0)").classed("time",1);
		...
		time.append("text").style("font-size","10px").attr({x:300,y:22,"text-anchor":"middle"}).text(night);
	...
	var itotal=indics.append("g");
		...
		itotal.append("text").attr({x:30,y:20,"text-anchor":"end"}).text(nbPeople).style("font-size","10px");
	...
	var vStats=svg.selectAll(".vStats").data([1]).enter().append("g").classed("vStats",1).attr("transform","translate(550,370)");
		...
		vStat.append("rect")
		.attr({width:(300/nbAttractions)-1,height:y,y:function(d) {return 125-y(d);}})
		.style("fill","steelblue");
		//vStats.append("text").attr("transform","translate("+((300/nbAttractions-1)/2)+",140)").attr("text-anchor","middle").text(function(d,i) {return i;}).style("font","8px");
		vStats.append("g")
    		.attr("class", "axis")
...
function draw() {
	...
	bars=svg.selectAll(".bar").select("rect").transition().duration(duration).attr({height:function(d) {return y(d.nb);},y:function(d) {return 125-y(d.nb);}});
	vStats=svg.selectAll(".vStat").data(visitStats).select("rect").transition().duration(duration).attr({height:y,y:function(d) {return 125-y(d);}})
	d3.select("#timetick").text(tick).attr("x",50+250*(tick/night));
	d3.select(".time").select(".cursor").attr("transform","translate("+(50+250*(tick/night))+",10)");
